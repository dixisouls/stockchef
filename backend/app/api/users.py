...
from app.schemas.user import DietaryPreferenceSchema, CuisineSchema, UserProfile, UserPreferenceUpdate
...
async def get_preferences(db: Session = Depends(get_db)):
    ...
    return {
        "dietary_preferences": [DietaryPreferenceSchema.model_validate(dp) for dp in dietary_prefs],
...
async def update_preferences(
    ...
    dietary_pref = db.query(DietaryPreference).filter(
        DietaryPreference.preference_id == preferences.dietary_preference_id
    ).first()
    if not dietary_pref:
        raise HTTPException(
    ...
    cuisine_pref = db.query(Cuisine).filter(
        Cuisine.cuisine_id == preferences.cuisine_preference_id
    ).first()
    if not cuisine_pref:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
