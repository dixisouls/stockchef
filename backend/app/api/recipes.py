...
from app.db.models import InventoryItem, Recipe, RecipeIngredient, User, UserRecipeHistory
from app.schemas.recipe import (RecipeCreate, RecipeDetail, RecipeSchema,
                              RecipeSuggestion, RecipeSuggestionRequest)
...
async def cook_recipe(
    ...
    history_entry = db.query(UserRecipeHistory).filter(
        UserRecipeHistory.user_id == current_user.user_id,
    ...
    ingredients_used = 0
    ingredient_names = [ingredient.ingredient_name.lower() for ingredient in recipe.ingredients]
